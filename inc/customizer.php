...
function ceylon_blog_customize_register( $wp_customize ) {
	...
	$wp_customize->add_section(
		'theme_options', array(
			'title'    => __( 'Theme Options', 'ceylonblog' ),
	...
	if ( function_exists( 'ceylon_blog_lazyload_images' ) ) {
		...
		$wp_customize->add_control(
			'lazy_load_media', array(
				'label'           => __( 'Lazy-load images', 'ceylonblog' ),
				'section'         => 'theme_options',
				'type'            => 'radio',
				'description'     => __( 'Lazy-loading images means images are loaded only when they are in view. Improves performance, but can result in content jumping around on slower connections.', 'ceylonblog' ),
				'choices'         => array(
					'lazyload'    => __( 'Lazy-load on (default)', 'ceylonblog' ),
					'no-lazyload' => __( 'Lazy-load off', 'ceylonblog' ),
				),
			)
		);
	}
...
function ceylon_blog_sanitize_lazy_load_media( $input ) {
	$valid = array(
		'lazyload' => __( 'Lazy-load images', 'ceylonblog' ),
		'no-lazyload' => __( 'Load images immediately', 'ceylonblog' ),
	);
