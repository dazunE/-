	...
	if ( is_home() && ! is_front_page() ) {
		...
		<header class="page-header">
			...
			<?php
				...
				printf( esc_html__( 'Search Results for: %s', 'ceylonblog' ), '<span>' . get_search_query() . '</span>' );
			?>
			</h1>
		</header><!-- .page-header -->
		<?php
...
function ceylon_blog_posted_on() {
	...
	$posted_on = sprintf(
		...
		esc_html_x( 'Posted on %s', 'post date', 'ceylonblog' ),
...
function ceylon_blog_posted_by() {
	$byline = sprintf(
		...
		esc_html_x( 'by %s', 'post author', 'ceylonblog' ),
...
function ceylon_blog_post_categories() {
	...
	if ( 'post' === get_post_type() ) {
		...
		$categories_list = get_the_category_list( esc_html__( ', ', 'ceylonblog' ) );
		if ( $categories_list ) {
			/* translators: 1: list of categories. */
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'ceylonblog' ) . ' </span>', $categories_list ); // WPCS: XSS OK.
		}
	}
}
...
function ceylon_blog_post_tags() {
	...
	if ( 'post' === get_post_type() ) {
		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'ceylonblog' ) );
		if ( $tags_list ) {
			/* translators: 1: list of tags. */
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'ceylonblog' ) . ' </span>', $tags_list ); // WPCS: XSS OK.
		}
	}
}
		...
		comments_popup_link(
			sprintf(
				wp_kses(
					...
					__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'ceylonblog' ),
					array(
						'span' => array(
							'class' => array(),
						),
	...
	edit_post_link(
		sprintf(
			wp_kses(
				...
				__( 'Edit <span class="screen-reader-text">%s</span>', 'ceylonblog' ),
				array(
					'span' => array(
						'class' => array(),
					),
...
function ceylon_blog_attachment_in( $post ) {
	if ( ! empty( $post->post_parent ) ) :
		$postlink = sprintf(
			...
			esc_html_x( 'in %s', 'original post', 'ceylonblog' ),
...
function ceylon_blog_the_attachment_navigation() {
	...
	<nav class="navigation post-navigation" role="navigation">
		<h2 class="screen-reader-text"><?php echo esc_html__( 'Post navigation', 'ceylonblog' ); ?></h2>
		<div class="nav-links">
			<div class="nav-previous">
				<div class="post-navigation-sub">
					<?php echo esc_html__( 'Previous attachment:', 'ceylonblog' ); ?>
				</div>
				<?php previous_image_link( false ); ?>
			</div><!-- .nav-previous -->
			<div class="nav-next">
				<div class="post-navigation-sub">
					<?php echo esc_html__( 'Next attachment:', 'ceylonblog' ); ?>
				</div>
